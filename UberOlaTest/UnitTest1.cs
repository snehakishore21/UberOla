...
namespace UberOlaTest
{
    ...
    public class UnitTest1
    {
        ...
        void testCabBookingFlow()
        {
            ...
            Assert.ThrowsException<NoCabsAvailableException>(() => {
                ridersController.book(r3, 0.0, 0.0, 500.0, 500.0);
            });



            Assert.ThrowsException<RiderNotFoundException>(() => {
                ridersController.book("abcd", 0.0, 0.0, 500.0, 500.0);
            });


            Assert.ThrowsException<CabAlreadyExistsException>(() => {
                cabsController.regiserCab("c1", "skjhsfkj");
            });

            Assert.ThrowsException<CabNotFoundException>(() => {
            ...
            Assert.ThrowsException<RiderAlreadyExistsException>(() => {
